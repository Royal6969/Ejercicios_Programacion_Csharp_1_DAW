...
namespace P35b_v2_Campos_Separados_A_Campos_Dimensionados
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            string[] log = new string[6]; // id $ Apellidos, Nombre $ movil $ fechaNac $ altura $ peso = 6 campos
            double suma = 0;
            ...
            for (int i = 0; i < LogsList.Count; i++)
            {
                ...
                if (log[0].Length < 3) {
                    streamWriter.Write(" " + CuadraTexto(log[0], 5));
                } else {
                    streamWriter.Write(CuadraTexto(log[0], 6));
                }
                ...
                (
                    ...
                    CuadraTexto(log[1], 34), 
                    CuadraTexto(log[2], 16), 
                    CuadraTexto(log[3].Substring(6, 2) + "/" + log[3].Substring(4, 2) + "/" + log[3].Substring(0, 4), 16),
                    CuadraTexto(log[4], 7)
                );

                if (log[5].Length < 5) {
                    streamWriter.WriteLine(" {0}", float.Parse(log[5]).ToString("00.00"), suma += Convert.ToDouble(log[5]));
                } else {
                    streamWriter.WriteLine("{0}", float.Parse(log[5]).ToString("00.00"), suma += Convert.ToDouble(log[5]));
                }
            }
